function [ data, g, data0 ] = etchingTest(accuracy, displayType, etchShape)
...
switch(etchShape)
    ...
    case 'triangle'
        ...
        if (g.dim > 2)
            ...
            % Velocity in the negative y-direction  
            % between -0.25 < x < 0.25, and 0 < y < 0.9
            vel{1,1}(g.xs{1,1}>-0.25 & g.xs{1,1} < 0 & g.xs{2,1}>0 & g.xs{2,1} < 0.9) = 0.5;
            vel{1,1}(g.xs{1,1}>= 0 & g.xs{1,1} < 0.25 & g.xs{2,1}>0 & g.xs{2,1} < 0.9) = -0.5;
            vel{2,1}(g.xs{1,1}>-0.25 & g.xs{1,1} < 0.25 & g.xs{2,1}>0 & g.xs{2,1} < 0.9) = -0.5;
        end
end
