function [ data, g, data0,width,height ] = etchingWithMask_new(accuracy, displayType, etchShape,current,i)
...
switch(etchShape)
    ...
    case 'varyAlpha'
        ...
        if (g.dim > 2)
            ...
            % between -0.25 < x < 0.25, and -0.25 < y < 0.25 & -0.25 < z
            vel{3,1} = zeros(size(g.xs{1,1}));
            vel{1,1}(g.xs{1,1}>-0.25 & g.xs{1,1} < 0 & g.xs{2,1}> -0.25 & g.xs{2,1} < 0.25) = -alpha*vel_d;
            vel{1,1}(g.xs{1,1}>0 & g.xs{1,1} < 0.25 & g.xs{2,1}> -0.25 & g.xs{2,1} < 0.25) = alpha*vel_d;
            vel{3,1}(g.xs{3,1} > -0.8) = vel_d;
                    
            % Mask (sort of):
            vel{3,1}(g.xs{3,1}>0.75 & g.xs{1,1} > 0.25) = 0;
            vel{3,1}(g.xs{3,1}>0.75 & g.xs{1,1} < -0.25) = 0;
            vel{3,1}(g.xs{3,1}>0.75 & g.xs{2,1} > 0.25) = 0;
            vel{3,1}(g.xs{2,1}>0.75 & g.xs{2,1} < -0.25) = 0;

            vel{2,1}(g.xs{3,1}>0.75 & g.xs{1,1} > 0.25) = 0;
            vel{2,1}(g.xs{3,1}>0.75 & g.xs{1,1} < -0.25) = 0;
            vel{1,1}(g.xs{3,1}>0.75 & g.xs{1,1} > 0.25) = 0;
            vel{1,1}(g.xs{3,1}>0.75 & g.xs{1,1} < -0.25) = 0;                   
        else
            % 2-dimensional: Velocity in the negative y-direction  
            % between -0.25 < x < 0.25, and 0 < y < 0.9
%              vel{1,1}(g.xs{1,1}>-0.6 & g.xs{1,1} < 0) = alpha*vel_d;
%              vel{1,1}(g.xs{1,1}>= 0 & g.xs{1,1} < 0.6) = -alpha*vel_d;
%              vel{2,1}(g.xs{2,1} > -0.8) = vel_d;
           %load vel_test.mat;
    ...
    case 'expAlpha'
        ...
        if (g.dim > 2)
            ...
            v_s (d < - 0.8) = 0;
            vel{3,1} = zeros(size(g.xs{1,1}));
            vel{1,1}(g.xs{1,1}>-0.25 & g.xs{1,1} < 0 & g.xs{2,1}> -0.25 & g.xs{2,1} < 0.25) = v_s(g.xs{1,1}>-0.25 & g.xs{1,1} < 0 & g.xs{2,1}> -0.25 & g.xs{2,1} < 0.25);
            vel{1,1}(g.xs{1,1}>0 & g.xs{1,1} < 0.25 & g.xs{2,1}> -0.25 & g.xs{2,1} < 0.25) = -v_s(g.xs{1,1}>0 & g.xs{1,1} < 0.25 & g.xs{2,1}> -0.25 & g.xs{2,1} < 0.25);
            vel{3,1}(g.xs{1,1}>-0.25 & g.xs{1,1} < 0.25 & g.xs{2,1}> -0.25 & g.xs{2,1} < 0.25 & g.xs{3,1}>-0.25) = vel_d;
            ...
            v_s = alpha*exp(d/lambda)*vel_d;
            vel{1,1}(g.xs{1,1}>-0.7 & g.xs{1,1} < 0) = v_s(g.xs{1,1}>-0.7 & g.xs{1,1} < 0);
            vel{1,1}(g.xs{1,1}>= 0 & g.xs{1,1} < 0.7) = -v_s(g.xs{1,1}>= 0 & g.xs{1,1} < 0.7);
            vel{2,1}(g.xs{2,1} > -0.8) = vel_d;
            % Mask (sort of):
            vel{2,1}(g.xs{2,1}>0.75 & g.xs{1,1} > 0.25) = 0;
            vel{2,1}(g.xs{2,1}>0.75 & g.xs{1,1} < -0.25) = 0;
...
while(tMax - tNow > small * tMax)
  ...
  [width, height] = getEtchDims(g, data);
endTime = cputime;
fprintf('\nTotal execution time %g seconds\n', endTime - startTime);
end

function [bottomWidth, centerHeight] = getEtchDims(g, data)

intfcPoints = isNearInterface(data);
%intfcData = zeros(size(data));
w = zeros(size(data,1),1);
h = zeros(size(data,2),1);
%intfcData(intfcPoints == 1) = data(intfcPoints == 1);
intfcPoints(g.xs{1,1} > 0.6) = 0;
intfcPoints(g.xs{1,1} < -0.6) = 0;
intfcPoints(g.xs{2,1} < -0.7) = 0;
intfcPoints(g.xs{2,1} > 0.79) = 0;
intfcPoints(g.xs{2,1} > 0.8) = 0;
intfcPoints(g.xs{2,1} >= 0.8) = 0;
intfcPoints(g.xs{2,1} >= 0.78) = 0;


center = round(size(data,1)/2);
for i = 1:size(data,1)
    top_end = 0; bottom_end = 0;
    bottom_ind = 0; top_ind = 0;
    ind = find(intfcPoints(i,:));
    
    if(size(ind,2) == 2)
        bottom_ind = ind(2);
        bottom_end = data(i,ind(2));
        top_ind = 181; top_end = 0;
    end
