...
if (nargin == 0) && (nargout == 0)
  ...
  fprintf('               a: [ constant; motion in normal direction  ]\n');
  fprintf('               b: [ constant; motion by mean curvature    ]\n');
    ...
    switch(lower(name))
     case 'a'
      if(isa(value, 'double') && (prod(size(value)) == 1))
        ...
        error('a must be a scalar double');
      end
     ...
     case 'b'
      if(isa(value, 'double') && (prod(size(value)) == 1))
        ...
        error('b must be a positive scalar double');
      end
     ...
     case 'dc'
      if(isa(value, 'double') && (prod(size(value)) == 1))
        ...
        error('dc must be a scalar double');
      end
