...
if( g.dim == 2 )
    opposite_side = findOppSide(mask,g,data0);
...
if( nargin < 2)
    ...
    a_entry = options.b * ((g.dim - 1)/(Rt*g.dx(1))); %breakthrough pressure
    ...
    a0 = 0.75*a_entry;  
    options1 = setOptions(options,'a',a0,'doDisplay',0,'doSave',0, 'doreinit',0);
...
amax = 5*a0; %arbitrary as well; it's set so the run is limited
